...
function buildQR(seed: string): boolean[][] {
    ...
    for (let r = 0; r < QR_N; r++)
        for (let c = 0; c < QR_N; c++)
            g[r][c] =
                (seed.charCodeAt((r * QR_N + c) % seed.length) ^
                    (r * 7 + c * 11)) %
                    3 !==
                0
...
function PsaShieldBadge({ size = 48 }: { size?: number }) {
    return (
        <svg width={size} height={size * 1.15} viewBox="0 0 48 55" fill="none">
            <path
                d="M24 53 C24 53, 3 41, 3 23 L3 5 C3 5, 14 6, 24 3 C34 6, 45 5, 45 5 L45 23 C45 41, 24 53, 24 53 Z"
                fill="#8c9aaa"
            />
            <path
                d="M24 47 C24 47, 8 37, 8 23 L8 9 C8 9, 16 10, 24 8 C32 10, 40 9, 40 9 L40 23 C40 37, 24 47, 24 47 Z"
                fill="#9eadb8"
                opacity="0.45"
            />
            <path
                d="M24 8 C16 10, 8 9, 8 9 L8 16 C8 16, 16 14, 24 12 C32 14, 40 16, 40 16 L40 9 C40 9, 32 10, 24 8 Z"
    ...
    return (
        <div
            style={{
                ...
                overflow: 'hidden',
                background:
                    'linear-gradient(160deg, rgba(212,220,230,0.92) 0%, rgba(175,188,200,0.88) 100%)',
                boxShadow:
                    '0 10px 40px rgba(0,0,0,0.5), 0 2px 8px rgba(0,0,0,0.25), inset 0 1px 0 rgba(255,255,255,0.6)',
        ...
        >
            ...
            <div
                style={{
                    ...
                    height: BANNER_H,
                    background:
                        'linear-gradient(150deg, #132050 0%, #1b2d6a 60%, #1a2860 100%)',
                    display: 'flex',
                    flexDirection: 'column',
                }}
            >
                ...
                >
                    <svg
                        viewBox={`0 0 ${totalW} 9`}
                        style={{ height: 9, width: 52, flexShrink: 0 }}
            ...
            <div
                style={{
                    ...
                    flexShrink: 0,
                    background:
                        'linear-gradient(90deg, transparent, rgba(180,170,160,0.6) 20%, rgba(180,170,160,0.6) 80%, transparent)',
                    margin: '5px 0',
                    position: 'relative',
                    zIndex: 2,
                }}
...
export function Card3DViewer({ uc, width = 260 }: Props) {
    ...
    const slabEdge = isGraded
        ? 'rgba(195,210,225,0.82)'
        : 'rgba(225,222,218,0.82)'
                ...
                >
                    ...
                    >
                        {isGraded ? (
                            ...
                            <img
                                src={
                                    uc.cards.image_url_hi ?? uc.cards.image_url
                                }
                                alt={uc.cards.name}
                                draggable={false}
                                onDragStart={(e) => e.preventDefault()}
