...
export default function BackfillTypesTab() {
    ...
    async function run() {
        ...
        setError(null)
        try {
            const res = await fetch('/api/admin/backfill-pokemon-types', { method: 'POST' })
    ...
    return (
        <div style={{ maxWidth: 480 }}>
            <h2 style={{ fontSize: '0.88rem', fontWeight: 700, color: '#e2e8f0', margin: '0 0 6px' }}>
                Backfill Pokemon Types
            </h2>
            <p style={{ fontSize: '0.7rem', color: '#64748b', margin: '0 0 16px', lineHeight: 1.5 }}>
                Fetches <code style={{ color: '#94a3b8' }}>pokemon_type</code> from PokeAPI for cards
                where it&apos;s currently NULL. Runs 15 cards per batch — press repeatedly until Done.
            </p>
            ...
            >
                {loading ? 'Running...' : isDone ? 'Done ✓' : 'Run Batch (15 cards)'}
            </button>
